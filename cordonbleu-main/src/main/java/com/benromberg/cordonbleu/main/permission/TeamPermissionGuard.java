...
public class TeamPermissionGuard {
    ...
    public UserTeam guardUpdateTeam(UserWithPermissions user, String teamId) {
        return user.hasTeamPermission(TeamPermission.MANAGE, teamId).orElseThrow(ForbiddenException::new);
    }

    public Team guardGetTeam(UserWithPermissions user, String name) {
        Optional<UserTeam> teamFromUser = user.getTeams().stream().filter(userTeam -> userTeam.getTeam().getName().equals(name)).findAny();
    ...
    public UserTeam guardGroupAssignment(UserWithPermissions user, String teamId) {
        return user.hasTeamPermission(TeamPermission.ASSIGN, teamId).orElseThrow(ForbiddenException::new);
    }
}
